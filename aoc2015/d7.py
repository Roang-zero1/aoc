...
def solve_circuits(circuits, values):
    while circuits:
        working_circuits = {k:x for k, x in circuits.items() \
...
def main():
    regex = re.compile(r'((?P<value>[0-9]+)|(((?P<o_variable>[a-z]+)|'
    ...
    values = {}
    circuits = {}
    with open('d7.txt') as input_file:
            ...
            if match.group('value'):
                ...
                if operation == 'LSHIFT' or operation == 'RSHIFT':
                    ...
                    circuit = Circuit(
                        ...
                        var2=match.group('o_variable') if match.group('o_variable') else None,
                        num1=uint16(match.group('number')) if match.group('number') else None,
                        num2=uint16(match.group('o_number')) if match.group('o_number') else None)
    ...
    print(f'Value of a after first round: {values["a"]}')
    ...
    print(f'Value of a after second round: {values["a"]}')
