...
class Rectangle:
    ...
    def is_overlapping(self, rect: "Rectangle") -> bool:
        ...
        ) and range_overlap(self.top, self.bottom, rect.top, rect.bottom)
    ...
    def overlap(self, rect: "Rectangle") -> "Rectangle":
        ...
        new_rect = Rectangle(
            left, top, right - left, bottom - top, Rectangle.create_ids(self, rect)
        )
...
compiled_re = re.compile(
    "#(?P<id>[0-9]+) @ (?P<left>[0-9]+),(?P<top>[0-9]+): (?P<width>[0-9]+)x(?P<height>[0-9]+)"
)
rectangles = []
...
def main(puzzle_input: List[str]):
    ...
    for rectangle in rectangles:
        overlaps_rect = [
            x
            for x in rectangles
            if rectangle.is_overlapping(x) and x.id != rectangle.id
        ]
    ...
    for id_a, overlap in overlaps.items():
        overlaps_rect = [
            v for k, v in overlaps.items() if overlap.is_overlapping(v) and k != id_a
        ]
    ...
    for overlap in overlaps:
        overlaps_rect = [
            x for x in overlaps if overlap.is_overlapping(x) and x != overlap
        ]
